...
if (Number(status.schemaVersion) !== 5 || status.format !== 'sharded-v3') throw new Error('Versão/formato do status incompatível');
if (Number(index.meta?.schemaVersion) !== 5 || index.meta?.format !== 'card-shard-index-v3') throw new Error('Índice de cartas inválido');
...
for (const file of status.shardFiles || []) {
  ...
  if (Number(payload.meta?.schemaVersion) !== 5 || payload.meta?.format !== 'price-shard-v3') throw new Error(`Shard inválido: ${file}`);
  ...
  for (const [key, item] of Object.entries(payload.prices || {})) {
    const expected = `${item.cardId}::${item.language}::${item.variantEnum}::${item.condition}`;
